...
def get_prefix_prompt(query, num):
    return [{'role': 'system',
             'content': "You are RankGPT, an intelligent assistant that can rank paths based on their relevancy to entity pair."},
            {'role': 'user',
             'content': f"I will provide you with {num} paths, each indicated by number identifier []. \nRank the paths based on their usefulness in inferring causal relationships between entity pair: {query}."},
            {'role': 'assistant', 'content': 'Okay, please provide the paths.'}]


def get_post_prompt(query, num):
    return f"Entity pair: {query}. \nRank the {num} paths above based on their usefulness in inferring causal relationships between entity pair. The paths should be listed in descending order using identifiers. The most relevant entity pair should be listed first. The output format should be [] > [], e.g., [1] > [2]. Only response the ranking results, do not say any word or explain."
